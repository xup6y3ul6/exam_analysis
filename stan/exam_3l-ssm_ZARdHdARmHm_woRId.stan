...
transformed parameters {
  ...
  for (i in 1:N) {
    for (j in 1:D) {
      omega[i, j, 1] = phi_m * omega_0[i, j] + sigma_omega_m * omega_raw[i, j, 1];
      for (k in 2:M) {
        omega[i, j, k] = phi_m * omega[i, j, (k-1)] + sigma_omega_m * omega_raw[i, j, k];
      }
    }
  }
...
model {
  ...
  for (i in 1:N) {
    ...
    for (j in 1:D) {
      // Level 1:
      y[i, j] ~ multi_normal_cholesky(rep_vector(mu_ij[i, j], M) + z[i, j] * b[i] + omega[i, j], L_Sigma);
      omega_raw[i, j] ~ std_normal();
      omega_0[i, j] ~ normal(0, 10);
    }
  }
...
generated quantities {
  ...
  for (i in 1:N) {
    for (j in 1:D) {
      y_hat[i, j] = rep_vector(beta + s[i], M) + z[i, j] * b[i];
    }
  }

  real rel_T = (psi_s^2) / (psi_s^2 + tau2_d + tau2_m + mean(sigma_epsilon^2)); // R_T
}
